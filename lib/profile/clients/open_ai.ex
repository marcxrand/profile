defmodule Profile.Clients.OpenAI do
  @model "gpt-5-mini"
  @embedding_model "text-embedding-3-small"

  ## EMBEDDINGS
  def embedding(input, model \\ @embedding_model) do
    post("/embeddings", %{input: input, model: model})
  end

  def embedding!(input, model \\ @embedding_model) do
    {:ok, %Req.Response{body: %{"data" => data, "model" => model}}} = embedding(input, model)

    embedding =
      data
      |> Enum.find(&Map.has_key?(&1, "embedding"))
      |> Map.get("embedding")
  ...
  ## PUBLIC API
  def chat!(input, json \\ %{}) do
    response!(input, json)
  end

  def search!(input, json \\ %{}) do
    json
    |> Map.put(:tools, [%{type: "web_search_preview"}])
    |> then(&response!(input, &1))
  end

  ## RESPONSES ENDPOINT
  defp response(input, json) do
    {schema, json} = Map.pop(json, :schema)
    ...
    |> put_schema?(schema)
    |> then(&post("/responses", &1))
  end

  defp response!(input, json) do
    {:ok, %Req.Response{body: %{"error" => nil, "output" => output}}} = response(input, json)

    output
    |> Enum.find(&Map.has_key?(&1, "content"))
    |> Map.get("content")
    |> Enum.map_join(" ", &Map.get(&1, "text"))
    |> then(&((json[:schema] && Jason.decode!(&1)) || &1))
  end

  ## POST
  defp post(url, body) do
    Req.new()
    |> Req.merge(auth: {:bearer, System.fetch_env!("OPENAI_API_KEY")})
    |> Req.merge(base_url: "https://api.openai.com/v1")
    |> Req.merge(json: body)
    |> Req.merge(url: url)
    |> Req.merge(receive_timeout: 900_000)
    |> Req.post()
  end

  ## HELPERS
  defp put_schema?(json, schema) do
    if schema do
      format = Profile.Clients.OpenAI.Responses.Schema.build_output(schema)
      Map.put(json, :text, %{format: format})
    else
      json
    end
  end
...
defmodule Profile.Clients.OpenAI.Responses.Schema do
  @moduledoc """
  https://raw.githubusercontent.com/vkryukov/openai_responses/refs/heads/main/lib/responses/schema.ex
  ...
  defp type_with_options?([type, opts])
       when (is_atom(type) or is_binary(type)) and (is_list(opts) or is_map(opts)), do: true
  ...
  defp build_from_normalized(%{
    ...
    built_properties =
      properties
      |> Map.new(fn {name, prop} -> {name, build_from_normalized(prop)} end)
